...
class GCNPolicy(object):
    ...
    def __init__(self, name, ob_space, ac_space, args, kind='small', atom_type_num=None, char_type_num=None):
    ...
    def _init(self, ob_space, ac_space, kind, atom_type_num, char_type_num, args):
        ...
        if args.is_train == 1:
            ...
            sample = cond_sample
        sample = tf.expand_dims(sample, axis=-1) #b*1*n*1
        ...
        fusion_emb = tf.layers.dense(concat_emb, args.emb_size, activation=None, use_bias=False, name='fusion')
        for i in range(args.fusion_layer_num-1):
            fusion_emb = tf.layers.dense(fusion_emb, args.emb_size, activation=None, use_bias=False, name='fusion_layer_'+str(i))
        # #if args.has_cond == 1:
        if args.has_cond == 1:
            emb_node = fusion_emb
        ...
        #if args.has_cond == 1:
        if args.is_train == 1:
            self._act = U.function([stochastic, ob['adj'], ob['node'], cond_smi, cond_sample], [self.ac, self.vpred, debug])
